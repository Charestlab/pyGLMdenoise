...
from glmdenoise.utils.fit_model import fit_model
from glmdenoise.utils.glm_predictresponses import glm_predictresponses
...
def glm_estimatemodel(design, data, stimdur, tr, hrfmodel, hrfknobs,
    ...
    for p in range(numruns):
        ...
        if not opt['extra_regressors'][p]:
            exmatrix.append(1)
        else:
            exmatrix.append(
                make_projection_matrix(
                    opt['extra_regressors'][p]))
        ...
        if not opt['extra_regressors'][p]:
            ...
            combinedmatrix.append(
                make_projection_matrix(
                    np.c_[pmatrix, opt['extra_regressors'][p]]
                )
            )
    ...
    # otherwise, do it as usual
        ...
        if hrfmodel in ['assume', 'optimize']:
            ...
            if results['betas'].ndim == 2:
                ...
                results['betasmd'] = np.median(results['betas'], 1)
