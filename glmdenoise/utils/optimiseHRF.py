...
def calccodStack(data, pred):
    ...
    nom_denom = []
    for y, yhat in zip(data, pred):
        # get run-wise nom denoms
        nom, denom = R2_nom_denom(y, yhat)
        nom_denom.append((nom, denom))

    # calculate global R2
    nom = np.array(nom_denom).sum(0)[0, :]  # extract 1st tuple
    denom = np.array(nom_denom).sum(0)[1, :]  # extract 2nd tuple
...
def olsmatrix_ulen(X, mode=None, verbose=True):
    """
    ...
    # do it
    if mode == 0:
        X, len = normalize(X[:, good], return_norm=True)
        temp = (np.diag(
            1/len
            ) @ (
                np.linalg.inv(
        ...
        temp = np.linalg.inv(X[:, good].T @ X[:, good]) @ X[:, good].T
